...
- [x] Upgrade hosted smoke into a real production artifact canary: Firebase phone signup, visible exam generation, Cloud Tasks worker completion, Exam PDF download, rasterized PDF inspection, and account cleanup on the deployed App Hosting origin.
- [x] Harden production worker startup and artifact generation after the canary caught real failures: lazy PDF/canvas imports with Node polyfills, standalone tracing for PDF parser native dependencies, single-pass LaTeX escaping, strict generated-question parsing/repair, and no silent generic fallback when model JSON is invalid.
...
- Post-mortem: the first production artifact canary exposed failures no local/unit gate could honestly prove: the worker crashed on top-level `pdf-parse`/`DOMMatrix`, then generated a generic fallback PDF when model JSON parsing failed. Permanent correction: hosted smoke now waits for worker completion, downloads the actual Exam PDF, and the agent must rasterize/inspect the artifact; worker code lazy-loads PDF dependencies, generated-question parsing accepts strict object/legacy array shapes, repairs once, and fails the job instead of silently shipping placeholder questions.
...
- Production artifact canary after worker/parser hardening passed: `TEST_BASE_URL=https://exampull-web--exampull-gpt-5-5.us-central1.hosted.app pnpm exec playwright test --config=playwright.prod.config.ts --project=desktop-chrome --workers=1 e2e/smoke.spec.ts` passed with 6 tests, including live phone signup, visible Generate, deployed Cloud Tasks completion, Exam PDF download, and account cleanup. Downloaded evidence is saved at `artifacts/smoke/phone-signup-generated-exam.pdf`; the first page was rasterized to `artifacts/smoke/rendered/phone-signup-generated-exam-page1.png` and visually inspected to confirm concrete cell membrane/osmosis/diffusion questions instead of fallback placeholders.
