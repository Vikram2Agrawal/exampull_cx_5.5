...
function isBuilderStepId(value: unknown): value is BuilderStepId {
...
export function NewExamForm({
	...
	const initialClass = classes.find((course) => course.id === initialClassId);
	...
	const [selectedMaterialIds, setSelectedMaterialIds] = useState<string[]>(
		initialClass?.materials.map((material) => material.id) ?? [],
	);
	...
	useLayoutEffect(() => {
		...
		try {
			...
			if (isBuilderStepId(draft.activeStep)) setActiveStep(draft.activeStep);
	...
	const missingReasons = [
		topics.length === 0
			? "add at least one topic or upload material with extracted topics"
			: null,
		cost > credits && !useScholarBoost ? `${cost - credits} more credits needed` : null,
		isUploadingSource ? "wait for uploads to finish" : null,
		mode === "power" && effectiveTier === "free" ? "Power Mode requires Scholar or Guru" : null,
		mode === "power" &&
		effectiveTier !== "free" &&
		!powerSlots.every((slot) => slot.topic.trim().length > 0)
			? "fill every Power Mode question topic"
			: null,
	...
	const canGenerate =
		...
		(mode === "standard" ||
			(effectiveTier !== "free" &&
				powerSlots.length > 0 &&
				powerSlots.every((slot) => slot.topic.trim().length > 0)));
	const courseLabel = (selectedClass?.name ?? className.trim()) || "No course yet";
	const sourceCount = selectedMaterialIds.length + sourceUploads.length;
	const stepItems: Array<{
		...
		},
		{
			...
			summary:
				mode === "power"
					? `${powerSlots.length} custom question${powerSlots.length === 1 ? "" : "s"}`
	...
	return (
		<form className="space-y-5" onSubmit={onSubmit}>
			<nav className="grid gap-2 sm:grid-cols-3" aria-label="Exam builder steps">
				{stepItems.map((item) => {
					const isActive = item.id === activeStep;
					const isComplete =
						(item.id === 1 && (title.trim() || sourceCount > 0 || className.trim())) ||
						(item.id === 2 && topics.length > 0) ||
						(item.id === 3 && canGenerate);
					...
					return (
						<button
							...
							className={cn(
								"flex min-h-20 items-start gap-3 rounded-xl border p-3 text-left transition sm:min-h-24 sm:p-4",
								isActive
									? "border-brand bg-brand/12 shadow-lg shadow-brand/10"
						...
						>
							...
							>
								{isComplete ? <Check aria-hidden="true" size={17} /> : item.id}
							</span>
							<span>
			...
			{tier === "free" && boostAvailable && activeStep !== 2 ? (
				<button
					type="button"
					className="w-full rounded-xl border border-premium/40 bg-premium/10 p-4 text-left text-sm"
				...
				>
					...
					<span className="mt-1 block text-muted">
						{boostUnlocked
							? "Available in Choose topics for one 25-question Scholar exam."
			...
			{activeStep === 1 ? (
				<div className="space-y-4">
						...
						<div className="grid gap-4 md:grid-cols-2">
							...
							<div>
								...
								<input
									...
									className="mt-2 h-11 w-full rounded-lg border border-glass-border bg-background/70 px-3 outline-none focus:ring-2 focus:ring-brand"
									placeholder="MATH 301 or AP Bio"
									maxLength={80}
								/>
							</div>
						</div>
											...
											>
												...
												<span>
													...
													<span className="block text-muted">
														{material.extractedTopics.length} extracted
														topics
													</span>
												</span>
											</label>
						...
						<div className="mt-5 rounded-lg border border-glass-border bg-background/45 p-4">
							<div>
								<div>
									...
									<p className="mt-1 text-sm text-muted">
										These files are used for this exam and saved on its record.
										Add them to a class first only if you want to reuse them
										later.
									</p>
								</div>
							</div>
							<div className="mt-4 grid gap-3 rounded-lg border border-dashed border-glass-border bg-background/45 p-3 text-sm sm:grid-cols-2">
								<button
									type="button"
									className="min-h-16 rounded-lg border border-brand/35 bg-brand/10 px-4 text-left font-medium"
									onClick={() => document.getElementById("typed-topics")?.focus()}
								>
									Type topics
									<span className="mt-1 block font-normal text-muted">
										Fastest path when you already know the chapters or concepts.
									</span>
								</button>
								<label
									htmlFor="exam-source-files"
									className="flex min-h-16 cursor-pointer items-center rounded-lg border border-glass-border bg-glass px-4 text-left text-sm font-medium hover:bg-glass-strong"
								>
									<FileUp
										aria-hidden="true"
									...
									<span>
										...
										<span className="mt-1 block font-normal text-muted">
											PDFs, slides, notes, photos, or scans.
										</span>
									</span>
								</label>
								<input
									id="exam-source-files"
							...
							<div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
								<label className="text-sm">
									<span className="font-medium">
										What should ExamPull focus on?
									</span>
									<input
										value={uploadFocus}
										onChange={(event) => setUploadFocus(event.target.value)}
										className="mt-2 h-11 w-full rounded-lg border border-glass-border bg-background/70 px-3 outline-none focus:ring-2 focus:ring-brand"
										placeholder="Optional: chapters 7-9 only"
										maxLength={500}
									/>
								</label>
								<label className="flex items-center gap-2 self-end rounded-lg border border-glass-border bg-background/45 px-3 py-3 text-sm">
									<input
										...
										onChange={(event) =>
											setUploadStyleReference(event.target.checked)
										}
									/>
										...
										>
											<div>
												...
												>
													{Math.max(
														1,
														Math.round(upload.sizeBytes / 1024),
													)}{" "}
													KB - {uploadStatusText(upload)}
												</p>
												{upload.focus ? (
												...
												{upload.extractedTopics.length > 0 ? (
													<p className="mt-1 text-muted">
														{upload.extractedTopics.length} topics
														extracted
													</p>
												) : null}
												{upload.extractionProgress ? (
													<div className="mt-2 max-w-xl">
														<div className="flex items-center justify-between gap-3 text-xs text-muted">
															<span>
																{upload.extractionProgress.detail}
															</span>
															<span>
											...
											<div className="flex items-center gap-2">
												...
												<button
													...
													onClick={() =>
														void removeSourceUpload(upload.id)
													}
												>
													<Trash2
														aria-label="Remove source upload"
														size={18}
													/>
												</button>
											</div>
					...
					<div className="flex justify-end rounded-xl border border-glass-border bg-glass p-4">
			...
			{activeStep === 2 ? (
				<div className="space-y-4">
					...
					>
						<div>
							<label className="text-sm font-medium" htmlFor="typed-topics">
								Topics to include
							</label>
							<textarea
								id="typed-topics"
					...
					<div className="flex flex-col-reverse justify-between gap-3 rounded-xl border border-glass-border bg-glass p-4 sm:flex-row sm:items-center">
			...
			{activeStep === 3 ? (
				...
				>
					{missingReasons.length > 0 ? (
						<div
							className="mb-4 rounded-lg border border-warning/35 bg-warning/10 p-3 text-sm leading-6 text-muted"
							role="status"
							aria-live="polite"
						>
						...
						<div
							className="mb-4 rounded-lg border border-success/30 bg-success/10 p-3 text-sm leading-6 text-success"
							role="status"
							aria-live="polite"
						>
							Ready to generate.
									...
									>
										...
										<div className="space-y-2">
											...
											<button
												...
												onClick={() =>
													setAllPowerSlots({ style: slot.style })
												}
											>
												Set all
											</button>
					...
					<div className="flex flex-col justify-between gap-3 rounded-xl border border-glass-border bg-glass p-4 sm:flex-row sm:items-center">
						<div>
