...
const jobName = process.env.PREVIEW_PURGE_SCHEDULER_JOB?.trim() || "purge-expired-previews";
const schedule = process.env.PREVIEW_PURGE_SCHEDULE?.trim() || "17 3 * * *";
const timeZone = process.env.PREVIEW_PURGE_TIME_ZONE?.trim() || "Etc/UTC";
const limit = Number.parseInt(process.env.PREVIEW_PURGE_LIMIT?.trim() || "500", 10);

if (!Number.isInteger(limit) || limit < 1 || limit > 500) {
	throw new Error("PREVIEW_PURGE_LIMIT must be an integer from 1 to 500.");
}
...
run(gcloud, [
	...
	timeZone,
	"--uri",
	`${webUrl}/api/workers/purge-expired-previews`,
	"--http-method",
	"POST",
	headersFlag,
...
console.log(
	`Preview purge scheduler ${exists ? "updated" : "created"}: ${jobName} (${schedule} ${timeZone}).`,
);
